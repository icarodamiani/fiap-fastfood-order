...
public class MessagingAdapter implements MessagingPort {
    ...
    @Override
    public Flux<Message> read(String queue) {
        return receive(queue)
            .map(ReceiveMessageResponse::messages)
            .flatMapMany(messages ->
                Flux.fromIterable(messages)
                    .doOnNext(message -> LOGGER.info("Received new message. {}", message))
            );
    }
    ...
    @Override
    public Mono<Message> ack(String queue, Message message) {
        return getQueueUrl().apply(queue)
    ...
    public <T> Mono<T> send(String queue, T payload, CheckedFunction1<T, String> serialize) {
        return Mono.just(serialize.unchecked().apply(payload))
            ...
            .map(unused -> payload)
            .switchIfEmpty(Mono.defer(() -> Mono.just(payload)
                .doOnSubscribe(subscription -> LOGGER.info("EMPTY RESPONSE FROM SQS."))));
    }
